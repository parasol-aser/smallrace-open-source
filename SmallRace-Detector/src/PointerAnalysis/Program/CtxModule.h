...
class CtxModule {
 private:
  ...
  inline std::pair<CallNodeTy *, bool> getOrCreateCallNode(const ctx *curCtx, const ctx *preCtx,
    ...
    */
    ...
    if (isEntryNode) {
      ...
      // llvm::outs() << "callGraph function:" << *F << "\n";
      ...
      auto result = ctxFunMap
                        ...
                        .second;  // CtxFunction
      assert(result);
      // llvm::outs() << "callGraph getTargetFun:" << *callNode->getTargetFun()->function << "\n";
    ...
    if (interceptResult.option == InterceptResult::Option::IGNORE_FUN) {
      return std::make_pair(nullptr, false);
    }



    //llvm::outs() << "callsite inst: " << *I << "\n";
    //llvm::outs() << "redirect to: " << *(interceptResult.redirectTo) << "\n";
    ...
    if (llvm::isa<llvm::Function>(interceptResult.redirectTo)) {
      ...
      // new node are inserted
      // llvm::outs() << "callGraph function:" << *F << "\n";
      // llvm::outs() << "callGraph Instruction:" << *I << "\n";
      CallNodeTy *callNode = callGraph->createCallNode(curCtx, F, I);
      ...
      //llvm::outs() << "indirect call: " << *indirect << "\n";

      // if(llvm::isa<llvm::CallBase>(indirect)) {
      //   auto secondBase = llvm::cast<CallBase>(indirect);
      //   llvm::outs() << "isa callbase: " << *(secondBase->getArgOperand(0)->stripPointerCasts()) << "\n";
      //   indirect = secondBase->getArgOperand(0);
      // }
  ...
  inline std::pair<CallNodeTy *, bool> getOrCreateInDirectCallNode(const ctx *C, const llvm::Instruction *I,
    ...
    // Smalltalk
    // llvm::outs() << "--- callgraph target=" << *target << "\n";
    // llvm::outs() << "--- callgraph Instruction=" << *I << "\n";
  ...
  void visitCallNode(CallNodeTy *node, std::queue<CallNodeTy *> &BFSQueue, BeforeNewNode beforeNewNode,
    ...
    for (const auto &BB : ctxFun) {
      for (const auto &inst : BB) {
        // llvm::outs() << inst << "\n";  
        if (llvm::isa<llvm::CallInst>(inst) || llvm::isa<llvm::InvokeInst>(inst)) { // only capture call in llvm
          ...
          // llvm::outs() << "--- cs.isIndirectCall():" << cs.isIndirectCall() << "\n";
          // cs.getCalledFunction()->isDeclaration()
          if (cs.isIndirectCall() ){ //|| cs.getCalledFunction()->isDeclaration() ) {
            // take another shot  
            ...
            // resolve correct function here? -- no, int updateFunPtrs
            // llvm::outs() << "-- visitCallNode:" << *fun << "\n";
            // auto targetNamePrefix = fun->getName().str();

            // auto targetfun = llvmModule->getFunction(targetNamePrefix + "$" + "SimpleRace");

            // bool isSmalltalk = true;
            // if(isSmalltalk) {
            //     auto targetNamePrefix = fun->getName().str();
            //     auto targetNameSuffix = llvm::cast<llvm::GlobalVariable>(llvm::cast<CallBase>(v)->getArgOperand(0)->stripPointerCasts())->getName().str();
            //     auto targetName = targetNamePrefix + "$" + targetNameSuffix;
            //     llvm::outs() << "Smalltalk function name: " << targetName << "\n";
            //     target = llvmModule->getFunction(targetName);
            //   }

            // llvmModule->getFunction()

            // if(targetfun){
            //   auto targetNode = visitDirectCallSite(calledCtx, context, targetfun, &inst, BFSQueue, beforeNewNode, onNewDirect,
            //                                       onNewInDirect);
            //   // node->insertEdge(targetNode, CallEdge(&inst));
            //   createEdge(node, targetNode, targetfun, &inst, onNewEdge);
            //   continue;
            // }
 ...
 public:
  ...
  void buildInitCallGraph(BeforeNewNode beforeNewNode, OnNewDirectNode onNewDirect, OnNewInDirectNode onNewIndirect,
    ...
    */
    ...
    if (entryFun == nullptr || entryFun->isDeclaration()) { // Return true if the primary definition of this global value is outside of the current translation unit.
      // https://llvm.org/doxygen/classllvm_1_1GlobalValue.html#a32e606ac4c88f71f14212e42b808e7f4
    ...
    auto entryNode =
        getOrCreateCallNode(initialCtx, initialCtx, entryFun, nullptr, beforeNewNode, onNewDirect, onNewIndirect).first;
  ...
  void resolveCallTo(CallGraphNode<ctx> *resolvedNode, const llvm::Function *target, BeforeNewNode beforeNewNode,
    ...
    */
    ...
    for (; it != et; it++) {
      ...
      //llvm::outs() << "target param at resolveCallTo:" << *target << "\n";
      std::pair<CallNodeTy *, bool> result;
      result = getOrCreateCallNode(calleeCtx, callerCtx, target, callInst, beforeNewNode, onNewDirect, onNewIndirect);
      //auto indirectSet = result.first->getTargetFunPtr()->getResolvedTarget();
      //llvm::outs() << *(*(indirectSet.begin())) << "\n";
      if (result.first == nullptr) {
        // the resolved function might then be ignored by the language model
        // or it replaced to another indirect call
        return;
      } else {
        // llvm::outs() << target->getName().str() << "\n";
        createEdge(callerNode, result.first, target, callInst, onNewEdge);
        if (!result.first->isIndirectCall() || target->getName().startswith("st.")) {
  ...
  [[nodiscard]] inline const CallGraphNode<ctx> *getDirectNodeOrNull(const ctx *C, const llvm::Function *F) {
    auto it = ctxFunMap.find(std::make_pair(C, F));
    if (it == ctxFunMap.end()) {
      // if (F->getName().startswith("st.")){ // move this part to ThreadTrace
      //   std::string newName = F->getName().str() + "$Race1"; // to do
      //   llvm::outs() << newName << "\n";
      //   const llvm::Function *newF = llvmModule->getFunction(newName);

      //   auto it2 = ctxFunMap.find(std::make_pair(C, newF));
      //   if (it != ctxFunMap.end()) {
          
      //     return it->second->getCallNode();
      //   }
      //   llvm::outs() << "Still not found at getDirectNodeOrNull\n";
      // }
      return nullptr;
    }
