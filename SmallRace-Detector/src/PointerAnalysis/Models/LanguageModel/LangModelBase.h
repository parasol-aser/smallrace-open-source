...
class LangModelBase : public ConsGraphBuilder<ctx, MemModel, PtsTy, SubClass> {
 public:
  ...
  bool isHeapAllocAPI(const llvm::Function *fun) { llvm_unreachable("SubClass should override the function!"); }
  // **** end ****
...
struct LangModelTrait<LangModelBase<ctx, MemModel, PtsTy, SubClass>> {
  ...
  static inline void addPreProcessingPass(llvm::legacy::PassManagerBase &passes) { MMT::addPreProcessingPass(passes); }
  ...
  [[nodiscard]] [[deprecated(
      "use getInDirectCallSite(ctx, instruction) "
