...
class ConsGraphBuilder : public llvm::CtxInstVisitor<ctx, SubClass>, public PtrNodeManager<ctx> {
 protected:
  ...
  struct OnNewCallEdge {
    Self &self;
    inline void operator()(const CallNodeTy *n1, const CallNodeTy *n2, const llvm::Function *originalTarget,
  ...
  bool initFunction(const CtxFunction<ctx> *fun) {
    ...
    for (const llvm::Argument &arg : fun->getFunction()->args()) {
      if (arg.getType()->isPointerTy()) {
        this->getOrCreatePtrNode(fun->getContext(), &arg);
      }
    }
  ...
  void initIndirectCall(const InDirectCallSite<ctx> *indirect) {
    ...
    //PtrNode *funPtrNode = getOrCreatePtrNode(indirect->getContext(), indirect->getValue());
  ...
  bool updateFunPtrs(const llvm::SparseBitVector<> &funPtrs
    ...
    for (NodeID id : funPtrs) {
      ...
      for (auto it = pointsTo.begin(), eit = pointsTo.end(); it != eit; it++) {
        ...
        if (auto objNode = llvm::cast<ObjNode>(node)) { // DBG: objNode->obj->memBlock->allocSite->value->dump()
          bool isSmalltalk = false;
          ...
          if(llvm::isa<CallBase>(object->getValue())) {
            ...
            if(calleeName.startswith("st."))
              isSmalltalk = true;
          }
          // if (isSmalltalk) {
          //   auto v = object->getValue(); // if it is correct, it should be a call to ObjectNew();
          //   llvm::outs() << "Smalltalk object value: " << *v << "\n";
          //   // auto callsite = objNode->getTargetFunPtr()->getCallSite();
          //   // auto targetNamePrefix = callsite->getCalledFunction()->getName().str();
          ...
          //   // target = module->llvmModule->getFunction(targetName);
          // } else {
          if (object->isFunction()) {
            target = llvm::dyn_cast<llvm::Function>(object->getValue());
          }
          // if (issmalltalk) {
          ...
          // auto object = objNode->getObject();
          // MAP TO THE FUNCTION =>
          // IF IT IS SMALL TALK, RESOLVE THE TYPE
          if (target || isSmalltalk) { // object->isFunction()
            ...
            for (CallGraphNode<ctx> *indirectNode : funPtrNode->getIndirectNodes()) {
              ...
              //llvm::outs() << "indirect callsite: " << *callsite << "\n";
              if(isSmalltalk) {
                auto v = object->getValue(); // if it is correct, it should be a call to ObjectNew();
                //llvm::outs() << "Smalltalk object value: " << *v << "\n";
                if(auto blk = dyn_cast<llvm::CallBase>(v)){
                  if(blk->getCalledFunction()->getName().startswith("st.model.blockParam")){
                    auto anonNameVar = llvm::cast<llvm::GlobalVariable>(llvm::cast<CallBase>(v)->getArgOperand(0)->stripPointerCasts());
                ...
                auto targetNamePrefix = (llvm::cast<CallBase>(callsite))->getCalledFunction()->getName();
                if(!llvm::isa<llvm::GlobalVariable>(llvm::cast<CallBase>(v)->getArgOperand(0)->stripPointerCasts()))
                  continue;
                auto targetNameSuffix = llvm::cast<llvm::GlobalVariable>(llvm::cast<CallBase>(v)->getArgOperand(0)->stripPointerCasts())->getName();
                auto targetName = targetNamePrefix.str() + "$" + targetNameSuffix.str();
                if(targetNameSuffix.startswith("st.anonfun")){
                  auto anonNameVar = llvm::cast<llvm::GlobalVariable>(llvm::cast<CallBase>(v)->getArgOperand(0)->stripPointerCasts());
                  auto anonNameArr = llvm::cast<llvm::ConstantDataArray>(anonNameVar->getInitializer());
                  auto anonFunc = this->getLLVMModule()->getFunction(anonNameArr->getAsString());
                  target = anonFunc;
                  ...
                  target = module->llvmModule->getFunction(targetName);
                if(!target) continue;
                //llvm::outs() << "Smalltalk function name: " << target->getName() << "\n";
              }

              assert(target);
              //llvm::outs() << "++ Target = " << *target << "\n";
              if (indirectNode->getTargetFunPtr()->isInterceptedCallSite()) {
                if (!isSmalltalk && !static_cast<SubClass &>(*this).isCompatible(callsite, target)) {
                  continue;
                }
              } else if (!isSmalltalk && !isCompatibleCall(callsite, target)) {
                continue;
              }
              ...
              bool newTarget = indirectNode->getTargetFunPtr()->resolvedTo(target, false);
              // llvm::outs() << "newtarget: " << newTarget << "\n";
              if (newTarget) {
                module->resolveCallTo(indirectNode, target, beforeNewNode, onNewDirect, onNewInDirect, onNewEdge);
  ...
  void processCallSite(const CtxFunction<ctx> *caller, const CtxFunction<ctx> *callee,
    ...
    assert(CS.isIndirectCall() || CS.getCalledFunction() == callee->getFunction() || CS.getCalledFunction()->getName().startswith("st.") );
    ...
    if (callee->getFunction()->isDeclaration()) {
      if (callee->getFunction()->getName().startswith("__cr_")) {
        // marker function for verifying the result of the pointer analysis
        // e.g., "__cr_no_alias__", "__cr_alias__"
        return;
      }
  ...
  void addGlobals() {
    ...
    // llvm::errs() << "------------  module->getLLVMModule()" << "\n"; 
    // llvm::errs() <<  module->getLLVMModule() << "\n";
  ...
  void addLocals() {
    ...
    // graph construction
    module->buildInitCallGraph(beforeNewNode, onNewDirect, onNewInDirect, onNewEdge); // src/PointerAnalysis/Program/CtxModule.h
  }
