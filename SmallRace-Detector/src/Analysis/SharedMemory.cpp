...
SharedMemory::SharedMemory(const ProgramTrace &program) {
  ...
  for (auto const &thread : program.getThreads()) {
    ...
    for (auto const &event : thread->getEvents()) {
      switch (event->type) {
        ...
        //   auto extcallEvent = llvm::cast<WriteEvent>(event.get());
        //   auto const ptsTo = extcallEvent->getAccessedMemory();
        //   if (DEBUG_PTA) {
        //     if (ptsTo.empty()) {
        //       llvm::outs() << "Write: ID " << extcallEvent->getID();
        //       extcallEvent->getIRInst()->getInst()->print(llvm::outs());
        //       llvm::outs() << ", empty pts, "
        //                    << "\n";
        //       break;
        //     } else {
  ...
  llvm::outs() << "+=+=+= SharedMemory has " << objReads.size() << " objects\n"; 
