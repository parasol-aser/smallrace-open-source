...
class EmbedCExprAST : public ExprAST {
  std::string name;

 public:
  EmbedCExprAST(Location loc, llvm::StringRef name)
      : ExprAST(Expr_EmbedC, loc), name(name) {}

  string getName() { return name; }

  /// LLVM style RTTI
  static bool classof(const ExprAST* c) { return c->getKind() == Expr_EmbedC; }
};
...
class ClassAST : public BaseAST {
 public:
  ...
  string class_name = "", environment = "", super_class = "", privateinfo = "",
         indexed_type = "";
  vector<std::string> inst_vars;
  vector<std::string> class_inst_vars;
  string imports = "";
  string category = "";
...
class ModuleAST : public BaseAST {
 ...
 public:
  ...
  void addClassAST(ClassAST* classAst) { classes.push_back(classAst); }
  std::vector<FunctionAST*> getFunctions() { return functions; }
  std::vector<ClassAST*> getClasses() { return classes; }
};
...
class BlockExprAST : public ExprAST {
 ...
 public:
  ...
  BlockExprAST(Location loc, const std::string& source,
               vector<VarDeclExprAST>& params)
      : ExprAST(Expr_Block, loc), source(source), params(params) {}
...
typedef struct {
  ...
  vector<VarDeclExprAST> blkparams;
